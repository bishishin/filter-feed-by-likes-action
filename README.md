...
元ソースをたどれるように、出力フィードには入力フィードのタイトルとリンクを残す。`rights` には生成年、入力フィードのタイトルおよびリンクを記載する。各記事では、タイトル、リンク、更新日時、公開日時、および入力フィードで `creator` として取得できる作成者名を残す。

転載としての権利周りを懸念し元フィード内の記事概要等は引き継がない。
